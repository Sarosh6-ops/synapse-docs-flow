...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    const participantsFilter = activeChatId === 'ai-assistant' ? [user.uid] : ['general-group'];

    const q = query(
        collection(db, 'messages'),
        where('chatId', '==', activeChatId),
        where('participants', 'array-contains-any', participantsFilter),
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const userMessage = {
      ...
      participants: activeChatId === 'ai-assistant' ? [user.uid] : ['general-group', user.uid]
    };
